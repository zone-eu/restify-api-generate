...
function parseJoiObject(path, joiObject, requestBodyProperties) {
  if (joiObject.type === "object") {
    ...
    if (
      joiObject.$_terms &&
      joiObject.$_terms.examples &&
      joiObject.$_terms.examples.length > 0
    ) {
...
async function generateAPiDocs(routes, options) {
    ...
    tags: options.tags || [
      {
        name: "Example tag",
        description:
          "This is an example tag provided if you do not specify any tags yourself in the options",
      },
    ],
  };
  ...
  for (const routePath in routes) {
    ...
    if (
      spec.validationObjs?.requestBody &&
      Object.keys(spec.validationObjs.requestBody).length > 0
    ) {
      ...
      parseJoiObject(
        "schema",
        Joi.object(spec.validationObjs?.requestBody),
        operationObj.requestBody.content[applicationType]
      );
    }
    ...
    for (const paramKey in spec.validationObjs?.queryParams) {
      ...
      if (
        paramKeyData.$_terms &&
        paramKeyData.$_terms.examples &&
        paramKeyData.$_terms.examples.length > 0
      ) {
  ...
  for (const path in mapPathToMethods) {
    ...
    for (const httpMethod in pathData) {
      ...
      for (const key in innerData?.requestBody?.content[
        Object.keys(innerData.requestBody.content)[0]
      ].schema.properties) {
        const reqBodyData =
          innerData.requestBody.content[
            Object.keys(innerData.requestBody.content)[0]
      ...
      for (const key in innerData.responses) {
        ...
        const obj =
          innerData.responses[key].content[
            Object.keys(innerData.responses[key].content)[0]
          ].schema;
  ...
  await fs.promises.writeFile(
    __dirname + options.docsPath || "/openapidocs.json",
    JSON.stringify(docs)
  );
}
